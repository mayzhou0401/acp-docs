...
* A1 series instances do not support deploying the EBS block storage plugin (Amazon EBS CSI Driver). If you need to use EBS block storage class storage classes, avoid using the following instance types:
...
4. Click **Create**.
    ...
    **Note**: If the default storage classes don't meet requirements, create new storage classes following the above steps and modify parameters as needed. See [Available Storage Class Parameters](#storage1).
